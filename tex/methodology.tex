...
\begin{figure}[!htbp]
\centering
\subfigure[RGB image]{\includegraphics[width=0.45\linewidth]{figures/methodology/inpainting_rgb.pdf}}
...
\mathbf{Ax=b}
\end{equation}
Assuming $m$ is the number of pixel inside $\Omega$ and $n$ is the sum of $m$ and the number of neighbouring pixel around the boundary $\delta \Omega$, $\mathbf{A}$ is a $n\times m$ Laplacian matrix, $\mathbf{b}$ is a $n\times1$ vector containing all the known boundary depth values and the $0$ inside $\Omega$.
Solving the linear equation with simple least squares, we can acquire the inpainted values.
With this naive image inpainting algorithm, we can fill the holes on the rough depth image as shown in Fig.~\ref{fig:pre-processing}.
...
The advantage of bilateral filter is reducing the noise while preserving the edge in the input image. 
...
\end{equation}
where $\hat{I}(\mathbf{x})$ is the filtered value at pixel $\mathbf{x} = (x,y)$, $\mathcal{N}$ represents the neighbouring pixels with $\mathbf{x}$ in the center, and $W$ is the sum of the all the neighbouring weights centering in $\mathbf{x}$, $\sigma_r$ and $\sigma_d$ denote the parameters controlling the spatial and depth weighting respectively. 
The smoothed result on our input depth image is shown in Fig.~\ref{fig:pre-processing}.

\begin{figure}[!htbp]
\centering
\subfigure[Input depth]{\includegraphics[width=0.27\linewidth]{figures/methodology/inpainting_shape_before.pdf}}
\subfigure[After inpainting]{\includegraphics[width=0.27\linewidth]{figures/methodology/inpainting_shape_after.pdf}}
\subfigure[After smoothing]{\includegraphics[width=0.27\linewidth]{figures/methodology/smooth_shape_after.pdf}}
...
\end{equation}
This process only applies once at the beginning of the process since the least squares is not sensitive to the details on the surface, thus the estimation from the smooth surface is enough.
...
Indeed, plenty of real-world objects has piecewise smooth appearance as shown in Fig.~\ref{fig:rgbd_illustration}, which means most pieces of a layout are dominated by certain colors.
...
\begin{tabular}{c|c c c c}
 ...
 \end{tabular}}
\caption{Comparison for the estimated albedo under the synthetic data. Noted that here the light parameter is given. From left to right: the albedo obtained without regularization, albedo estimated by RGBD-Fusion like method, albedo from the proposed ratio model and the ground truth albedo.}
...
\subfigure[A color image with our lighting setup]{\includegraphics[width=0.45\linewidth]{figures/methodology/ratio_setup_view.pdf}}
...
\begin{equation}\label{eq:ratio_energy}
    ...
    \end{split}
\end{equation}
where $\mathcal{P}$ is the stack of RGB albedo, and $\mathscr{R}$ denotes our ratio model which will be defined separately in the following part.
This energy is successively composed of a proposed ratio shading term, a depth fidelity term, an albedo smoothness term, an albedo fidelity term and a SH estimation term.
Now we will explain the proposed algorithm based on a new ratio model.
...
In order to simulate the scenario that a direct light comes from different directions, we simply sway a white LED light in different directions and take several images (even just the flashlight on any phone is okay). An example of a vase from different lighting directions is shown in Fig.~\ref{fig:robust_setup}.

\begin{figure}[!htbp]
\centering
\subfigure{\includegraphics[width=0.23\linewidth]{figures/methodology/robust_setup1.pdf}}
\subfigure{\includegraphics[width=0.23\linewidth]{figures/methodology/robust_setup2.pdf}}
\subfigure{\includegraphics[width=0.23\linewidth]{figures/methodology/robust_setup3.pdf}}
\subfigure{\includegraphics[width=0.23\linewidth]{figures/methodology/robust_setup4.pdf}}
...
Let $\odot$ denote element-wise multiplication between a vector and each column of a matrix, we have matrices $\mathbf{A}_{\mathbf{s}_{i,c}} = \rho_c \odot \tilde{\mathbf{n}}(z)$ for every channel in all input color images. 
...
\begin{equation}\label{eq:robust_albedo_estimate}
    \min_{\rho_c} \; \lVert \mathbf{A}_{\rho_c}\rho_c - \mathbb{I}_c \rVert^2_2 
\end{equation}
%$$$$$$$$$$$$$$$$$
$ \mathbf{A}_{\rho_c} \in \mathbb{R}^{mn \times m}$ is the stack of $n$ diagonal matrices $\text{diag}(\tilde{\mathbf{n}} \cdot \mathbf{s}_{i,c})$, where $\text{diag} : \mathbb{R}^m \rightarrow \mathbb{R}^{m\times m}, i \in \{1, \cdots, n\}$, and $\mathbb{I}_c \in \mathbb{R}^{mn}$ denotes the stack of all the vectorized $I_i$ in channel $c$, which can be represented as:
\begin{equation}
    \mathbf{A}_{\rho_c} = \begin{pmatrix} \tilde{\mathbf{n}} \cdot \mathbf{s}_{1,c} & &\\ & \ddots & & \\ && \tilde{\mathbf{n}} \cdot \mathbf{s}_{n,c}\\\end{pmatrix} \; \;
    \mathbb{I}_c = \begin{pmatrix} I_{1,c} \\ \vdots \\ I_{n,c} \end{pmatrix}
\end{equation}
...
\begin{tabular}{c| c c c}
   \includegraphics[height = 0.2\linewidth]{figures/methodology/robust_rgb_shirt.pdf}&
   \includegraphics[height = 0.2\linewidth]{figures/methodology/rgbd_rho_shirt.pdf} &
   \includegraphics[height = 0.2\linewidth]{figures/methodology/robust_rho_shirt.pdf}&
   \parbox[b]{1cm}{\includegraphics[width=0.1\textwidth]{figures/methodology/rgbd_rho_shirt_crop.png}\\
   \includegraphics[width=0.1\textwidth]{figures/methodology/robust_rho_shirt_crop.png}}
...
%----------------------------------------------
We can notice that there exist over-smoothing problems in many 3D scanning applications, which is due to the fact that the depth map given by the consumer RGB-D camera usually has a really low resolution.
...
\subfigure[Estimated normal$(960\times1280)$]{\includegraphics[width=0.31\linewidth]{figures/methodology/sr_normal.pdf}}
